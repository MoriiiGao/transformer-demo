"""
输入emb后的词序列，根据Q, K, V方法计算词与词之间的相关性
...
class MHAttention(nn.Module):
    ...
    def forward(self, x_q, x_k_v, attn_mask):
        # [batch_size, seq_len, emb_size] -> [batch_size, seq_len, head * q_k_size]
        q = self.w_q(x_q)
        k = self.w_k(x_k_v)

        # 多头兼顾
        # q: [batch_size, seq_len, head * q_k_size] -> [batch_size, head, seq_len, q_k_size]
        q = q.view(q.size()[0], q.size()[1], self.head, self.q_k_size).transpose(1, 2)
        # k: [batch_size, seq_len, head * q_k_size] -> [batch_size, head, q_k_size, seq_len]
        k = k.view(k.size()[0], k.size()[1], self.head, self.q_k_size).transpose(1, 2).transpose(2, 3)

        # 注意力矩阵
        # q:[batch_size, head, seq_len, q_k_size] * k:[batch_size, head, q_k_size, seq_len]
        # attn: [batch_size, head, seq_len, seq_len]
...
if __name__ == "__main__":
    ...
    # 多头注意力
    multihead = MHAttention(emb_size=128, q_k_size=256, v_size=512, head=8)
